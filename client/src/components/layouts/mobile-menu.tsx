...
export function MobileMenu() {
  ...
  return (
    <div className="bg-white border-b border-gray-200 z-10 fixed top-0 left-0 right-0 md:hidden">
      <div className="px-4 py-3 flex items-center justify-between">
        <div className="flex items-center">
          <Sheet open={open} onOpenChange={setOpen}>
            <SheetTrigger asChild>
              <Button variant="ghost" size="sm" className="mr-2 text-gray-500">
                ...
                <div className="px-4 py-3 border-t border-b border-gray-200">
                  ...
                  {categories.map(category => (
                    <Link key={category.id} href={`/links?category=${category.id}`}>
                      <a 
                        className="flex items-center px-2 py-2 text-sm font-medium rounded-md text-gray-600 hover:bg-gray-50 group"
                        onClick={closeMenu}
                      >
                        <FontAwesomeIcon 
          ...
          <div className="flex items-center">
            <div className="w-7 h-7 rounded-md bg-primary flex items-center justify-center mr-2">
              <FontAwesomeIcon icon="link-slash" className="text-white text-sm" />
            </div>
            <h1 className="text-lg font-bold text-gray-900">Link Savor</h1>
          </div>
        </div>
        {user && (
          <div className="flex items-center">
